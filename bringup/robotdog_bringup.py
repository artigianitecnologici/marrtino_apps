    ...
    while dorun:
        ...
        if data is not None:
            if len(data)==0:
                ...
                print(data)
                pfolder = os.getenv('MARRTINO_APPS_HOME')+"/robotdog/launch/"
                if data=='@robotdog_start':
                    tmux.cmd(0,'cd %s' %pfolder)
                    tmux.cmd(0,'roslaunch start_dynamixel.launch')
                
                elif data=='@robotdog_kill':
                    tmux.Cc(0)
                else:
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='pantilt bringup')
